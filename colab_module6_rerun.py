...
try:
    ...
    raise RuntimeError(
        "❌ google.colab недоступен. Запускайте модуль 6 в Google Colab."
    ) from e

colab_files = files

# --- Проверки сессии (модуль 1 / 5 уже должны быть выполнены) ---
_required = [
    ("RAGIndex", "класс RAGIndex"),
    ("ask_deepseek", "функция ask_deepseek"),
    ("extract_text", "функция extract_text"),
    ("SUPPORTED_DOCS", "SUPPORTED_DOCS"),
    ("SUPPORTED_ARCHIVES", "SUPPORTED_ARCHIVES"),
]
_missing = [label for name, label in _required if name not in globals()]
if _missing:
    raise RuntimeError(
        "❌ В сессии нет: " + ", ".join(_missing) + ".\n"
        "   Сначала выполните модуль 5 (или модуль 1), затем модуль 6."
    )

_api = (os.environ.get("DEEPSEEK_API_KEY") or globals().get("DEEPSEEK_API_KEY") or "").strip()
if not _api:
    raise RuntimeError(
        "❌ API-ключ DeepSeek не найден в сессии.\n"
        "   Сначала выполните модуль 5 (или модуль 1) и введите ключ."
    )
os.environ["DEEPSEEK_API_KEY"] = _api
...
_T0_ALL = time.time()
_CURRENT_STATUS = "старт"


def _mark(name: str) -> None:
    _TIMINGS[name] = time.time()
...
def _elapsed(name_from: str, name_to: str = None) -> float:
    ...
    b = _TIMINGS.get(name_to, time.time()) if name_to else time.time()
    return b - a
...
def _status(msg: str) -> None:
    ...
    _CURRENT_STATUS = msg
    print(f"🔄 СТАТУС: {msg}  | ⏱ прошло {_fmt_dur(_elapsed('start'))}", flush=True)
...
_mark("start")
print("=" * 70)
print("🚀 МОДУЛЬ 6 — Повторный анализ тендера")
print("=" * 70)
print("Библиотеки и API-ключ берутся из текущей сессии (без переустановки).")
print()

# =============================================================================
# ЭТАП 1/4 — Очистка старых данных
# =============================================================================
_mark("clear_start")
_status("этап 1/4 — очистка uploaded_files и rag_index")
print("=" * 70)
print("ЭТАП 1/4 — Очистка старых данных")
print("=" * 70)

if "uploaded_files" in globals() and isinstance(uploaded_files, dict):
    _old_n = len(uploaded_files)
    uploaded_files.clear()
    print(f"   ✓ uploaded_files очищен (было файлов: {_old_n})")
else:
    uploaded_files = {}
    print("   ✓ uploaded_files создан заново")

if "rag_index" in globals() and rag_index is not None and hasattr(rag_index, "clear"):
    try:
        _old_chunks = len(getattr(rag_index, "chunks", []) or [])
    except Exception:
        _old_chunks = 0
    rag_index.clear()
    print(f"   ✓ rag_index.clear() (было чанков: {_old_chunks})")
else:
    # создадим пустой индекс; размер чанка задаст этап индексации
    _cs = int(globals().get("CHUNK_SIZE", 600) or 600)
    _co = int(globals().get("CHUNK_OVERLAP", 100) or 100)
    rag_index = RAGIndex(chunk_size=_cs, chunk_overlap=_co)
    print("   ✓ rag_index создан заново")

# сброс результатов прошлого анализа (если были)
for _name in (
    "tender_report", "tender_report_filename", "tender_number",
    "tender_answers", "tender_file_status", "tender_timings",
    "documents", "FILE_CLASSIFICATION", "APPROVALS",
):
    if _name in globals():
        globals()[_name] = None if _name != "APPROVALS" else []

_mark("clear_end")
print(f"⏱️ Очистка: {_fmt_dur(_elapsed('clear_start', 'clear_end'))}")

# =============================================================================
# ЭТАП 2/4 — Загрузка новых файлов
# =============================================================================
_mark("upload_start")
_status("этап 2/4 — загрузка новых файлов")
print("\n" + "=" * 70)
print("ЭТАП 2/4 — Загрузка новых файлов")
print("=" * 70)
...
if not uploaded_files:
    raise RuntimeError("❌ Файлы не загружены. Запустите модуль 6 ещё раз и выберите файлы.")
...
print("\n" + "=" * 70)
print("ЭТАП 3/4 — Извлечение текста, OCR, индексация")
print("=" * 70)
...
_mark("index_end")
print(f"⏱️ Индексация: {_fmt_dur(_elapsed('index_start', 'index_end'))}")

if not getattr(rag_index, "chunks", None):
    raise RuntimeError("❌ Индекс пуст — не удалось извлечь текст из загруженных файлов.")

# =============================================================================
# ЭТАП 4/4 — Анализ 86 вопросов (логика модуля 4)
# =============================================================================
_mark("analysis_start")
_status("этап 4/4 — анализ тендера (86 вопросов)")
print("\n" + "=" * 70)
print("ЭТАП 4/4 — Анализ тендера (86 вопросов)")
print("=" * 70)
...
# Алиасы статуса анализа (совместимость с кодом модуля 4)
_T0_ANALYSIS = time.time()
_fmt_elapsed = _fmt_dur
# _status и _manual_download_link уже определены выше
...
print("🚀 86 вопросов по всем файлам тендера (ускоренный режим)")
...
def _progress_bar(done: int, total: int, t0: float, title: str = "") -> str:
    ...
    passed = _fmt_elapsed(time.time() - _T0_ANALYSIS)
    return (
...
print("\n" + "=" * 70)
print("СВОДКА ВРЕМЕНИ — МОДУЛЬ 6")
print("=" * 70)
print(f"   1. Очистка:        {_fmt_dur(_elapsed('clear_start', 'clear_end'))}")
print(f"   2. Загрузка:       {_fmt_dur(_elapsed('upload_start', 'upload_end'))}")
print(f"   3. Индексация:     {_fmt_dur(_elapsed('index_start', 'index_end'))}")
print(f"   4. Анализ:         {_fmt_dur(_elapsed('analysis_start', 'analysis_end'))}")
print(f"   ─────────────────────────────")
print(f"   ИТОГО:             {_fmt_dur(_elapsed('start', 'end'))}")
print()
_status("модуль 6 завершён")
print("✅ Модуль 6 завершён. Можно снова запустить модуль 6 для следующего тендера.")
